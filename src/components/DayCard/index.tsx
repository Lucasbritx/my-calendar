...
const DayCard = ({ day, events, selectedPeriod, viewMode }: DayCardProps) => {
  ...
  const eventsWithPositions = dayEvents.map((event) => {
    ...
    const span = endMinutes === 0 ? endHour - startHour : endHour - startHour + 1;
  ...
  const eventsOverlap = (event1: any, event2: any) => {
    return event1.startTimeInMinutes < event2.endTimeInMinutes && 
           event2.startTimeInMinutes < event1.endTimeInMinutes;
  };

  const sortedEvents = [...eventsWithPositions].sort((a, b) => a.startTimeInMinutes - b.startTimeInMinutes);
  const eventPositions: Array<typeof eventsWithPositions[0] & { column: number }> = [];
  
  sortedEvents.forEach((event) => {
    let column = 1;
    
    const conflictingEvents = eventPositions.filter(positioned => eventsOverlap(event, positioned));
  ...
  const maxColumns = Math.max(1, ...eventPositions.map(event => event.column));
